...
import { checkGpgKeyring } from './keyDiagnostics.js';
 ...
 */
export async function searchKeys(query, options = {}) {
    try {
        const db = await keyManager.loadKeyDatabase();
        const results = [];
 ...
 */
export async function getKeyById(id, options = {}) {
    try {
 ...
 */
export async function readKeyContent(key, options = {}) {
    try {
        if (!key)
            return null;
 ...
 */
export async function exportGpgKey(keyId, options = {}) {
    try {
    ...
    try {
        ...
        switch (source) {
            ...
            case 'gpg-import': {
                ...
                const tempFile = path.join(await fsPromises.mkdtemp(path.join(homedir(), '.dedpaste-temp-')), 'key.asc');
 ...
 */
export async function getKeyReport(options = {}) {
    try {
        const db = await keyManager.loadKeyDatabase();
            ...
            if (gpgInfo.available) {
                ...
                if (gpgInfo.keys.length > 0) {
                    for (const key of gpgInfo.keys) {
                        ...
                        report += `  - User: ${uid}\n`;
                        if (key.created)
                            report += `  - Created: ${new Date(key.created).toLocaleString()}\n`;
                        if (key.expires)
                            report += `  - Expires: ${new Date(key.expires).toLocaleString()}\n`;
                        if (key.trust)
                            report += `  - Trust: ${key.trust}\n`;
                        report += '\n';
                    }
                }
            }
