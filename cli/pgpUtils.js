    ...
    try {
        ...
        if (signatures && signatures.length > 0) {
            ...
            for (const sig of signatures) {
                const keyId = sig.keyID ? sig.keyID.toHex() : 'unknown';
                if (sig.valid === true) {
...
async function validatePgpKey(pgpKeyString, isPrivate = false) {
    ...
    try {
        ...
        try {
            ...
            if (pgpKey.keyPacket) {
                const keyPacket = pgpKey.keyPacket;
...
async function decryptWithGpgKeyring(encryptedContent) {
    ...
    try {
        ...
        const decryptResult = await execFilePromise('gpg', ['--decrypt', '--batch', '--yes', tempFilePath], decryptOptions);
...
// Export functions
export { fetchPgpKey, importPgpKey, addPgpKeyFromServer, convertPgpKeyToPem, importPgpPrivateKey, encryptWithPgp, decryptWithPgp, createPgpEncryptedMessage, decryptPgpMessage, validatePgpKey, decryptWithGpgKeyring };
