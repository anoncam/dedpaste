...
// Extended type definitions for enhanced interactive mode
interface ModuleImports {
  unifiedKeyManager?: any;
  runKeyDiagnostics?: (options?: any) => Promise<any>;
  formatDiagnosticsReport?: (results: any) => string;
  encryptContent?: (content: string | Buffer, recipient?: string | null, isPgp?: boolean) => Promise<string | Buffer>;
  decryptContent?: (encryptedContent: string | Buffer, options?: any) => Promise<any>;
  continueWithEncryption?: (recipientId: string | null, message: string) => Promise<any>;
  safeCheckGpgKeyring?: (timeout?: number) => Promise<any>;
}
...
let modules: ModuleImports = {};

// Function to initialize all required modules
async function initModules(): Promise<boolean> {
  try {
    ...
    modules.encryptContent = encryptionModule.encryptContent as any;
    modules.decryptContent = encryptionModule.decryptContent as any;
    
    const helpersModule = await import('./encryptionHelpers.js');
    modules.continueWithEncryption = helpersModule.continueWithEncryption as any;
 ...
 */
export async function enhancedKeyManagement(): Promise<OperationResult> {
  try {
  ...
  try {
    const answers: InquirerAnswers = await inquirer.prompt([
      ...
      },
      {
        ...
        when: (currentAnswers: InquirerAnswers) => currentAnswers.searchMode !== 'all'
      }
    ]);
    ...
    // Group keys by type
    const groupedKeys: Record<string, KeySearchResult[]> = {
      self: [],
      friend: [],
      pgp: [],
      keybase: [],
...
export async function addOrImportKey(): Promise<void> {
  try {
    ...
    let options: ImportKeyOptions = { source: sourceAnswer.importSource as ImportKeyOptions['source'] };
    ...
    switch (sourceAnswer.importSource) {
      case 'file': {
        ...
        if (fileAnswers.name) options.name = fileAnswers.name;
        break;
      }
      ...
      case 'pgp-server': {
        const pgpAnswers: InquirerAnswers = await inquirer.prompt([
          {
            ...
            validate: (input: string) => input ? true : 'Identifier cannot be empty'
          },
          {
            type: 'input',
            name: 'name',
        ...
        if (pgpAnswers.name) options.name = pgpAnswers.name;
        break;
      }
      ...
      case 'keybase': {
        ...
        if (keybaseAnswers.name) options.name = keybaseAnswers.name;
      ...
      case 'gpg-keyring': {
        ...
        const gpgKeys = gpgInfo.gpg.keys.map((key: GpgKeyInfo, index: number) => {
        ...
        if (gpgAnswers.name) options.name = gpgAnswers.name;
        break;
      }
        ...
        const pasteAnswers: InquirerAnswers = await inquirer.prompt([
          {
            ...
            validate: (input: string) => input ? true : 'Key content cannot be empty'
          },
          {
            type: 'input',
            name: 'name',
    ...
    // Import the key
    const result: OperationResult = await modules.unifiedKeyManager?.importKey(options) || { success: false, error: 'Import failed' };
...
export async function generateNewKey(): Promise<void> {
  try {
    ...
    // Generate the key
    const result: OperationResult = await modules.unifiedKeyManager?.generateKey() || { success: false, error: 'Generation failed' };
    ...
    if (result.success) {
      ...
      if (backupAnswer.backup) {
        const backupDirAnswer: InquirerAnswers = await inquirer.prompt([{
          ...
          validate: (input: string) => input ? true : 'Directory cannot be empty'
        }]);
 ...
 */
export async function viewKeyDetails(preloadedKeys: KeySearchResult[] | null = null): Promise<void> {
  try {
    ...
    const keyChoices: InteractiveChoice[] = keys.map((key: KeySearchResult) => {
      let label = '';
    ...
    const keyInfo: KeySearchResult | null = await modules.unifiedKeyManager?.getKeyById(keyAnswer.selectedKeyId, { includeGpg: true }) || null;
    ...
    if (contentAnswer.viewContent) {
      ...
      const keyContent: string | null = await modules.unifiedKeyManager?.readKeyContent(keyInfo, { 
        private: viewPrivate 
      }) || null;
...
export async function exportKeys(): Promise<void> {
  try {
    // Search for all keys
    const keys: KeySearchResult[] = await modules.unifiedKeyManager?.searchKeys('', { includeGpg: true }) || [];
...
export async function removeKeys(): Promise<void> {
  try {
    ...
    const keys: KeySearchResult[] = await modules.unifiedKeyManager?.searchKeys('', { includeGpg: false }) || [];
    ...
    const keyInfo: KeySearchResult | null = await modules.unifiedKeyManager?.getKeyById(keyAnswer.selectedKeyId) || null;
    ...
    // Remove the key
    const result: OperationResult = await modules.unifiedKeyManager?.removeKey(keyAnswer.selectedKeyId) || { success: false, error: 'Removal failed' };
...
export async function runDiagnostics(): Promise<void> {
  try {
    ...
    const results: any = await modules.runKeyDiagnostics?.() || {
      status: 'error',
    ...
    const report: string = modules.formatDiagnosticsReport?.(results) || 'Report generation failed';
 ...
 */
async function fixDiagnosticIssues(diagnosticResults: any): Promise<void> {
  try {
    // Check for missing directories
    let fixedCount = 0;
    const missingDirs = Object.entries(diagnosticResults.filesystemChecks?.directories || {})
      .filter(([, info]) => !(info as any)?.exists)
      .map(([dir]) => dir);
 ...
 */
export async function enhancedInteractiveSend(): Promise<OperationResult> {
  try {
    ...
    const allKeys: KeySearchResult[] = await modules.unifiedKeyManager?.searchKeys('', { includeGpg: false }) || [];
    ...
    let recipientId: string | null = recipientAnswer.recipient ?? null;
    ...
    if (recipientAnswer.recipient === 'gpg') {
      ...
      const handleAlternativeEncryption = async (reason: string): Promise<OperationResult> => {
        console.log(chalk.yellow(`\n${reason}`));
      ...
      let gpgChoices: InteractiveChoice[] = [];
      try {
        gpgChoices = gpgInfo.keys.map((key: any) => {
...
export async function preloadEnhancedMode(): Promise<boolean> {
