...
import { KeyDatabase, KeyInfo, InteractiveChoice } from '../src/types/index.js';
...
// Type for inquirer prompts
interface KeyManagementPrompt {
  action: 'list' | 'add' | 'remove' | 'export' | 'cancel';
}

interface FriendNamePrompt {
  name: string;
}

interface MethodPrompt {
  method: 'file' | 'paste' | 'display' | 'cancel';
}
 ...
 */
async function interactiveKeyManagement(): Promise<InteractiveResult> {
  const { action } = await inquirer.prompt<KeyManagementPrompt>([{
    type: 'list',
    name: 'action',
 ...
 */
async function interactiveListKeys(): Promise<InteractiveResult> {
 ...
 */
async function interactiveAddFriend(): Promise<InteractiveResult> {
  const { name } = await inquirer.prompt<FriendNamePrompt>([{
    type: 'input',
    name: 'name',
  ...
  const { method } = await inquirer.prompt<MethodPrompt>([{
    type: 'list',
    name: 'method',
  ...
  if (method === 'file') {
    const { filePath } = await inquirer.prompt<FilePathPrompt>([{
      type: 'input',
      name: 'filePath',
    ...
    const { content } = await inquirer.prompt<ContentPrompt>([{
      type: 'editor',
      name: 'content',
 ...
 */
async function interactiveRemoveKey(): Promise<InteractiveResult> {
  const db = await listKeys();
  const friendNames = Object.keys(db.keys.friends);
  ...
  const choices: InteractiveChoice[] = friendNames.map(name => ({ name, value: name }));
  choices.push({ name: 'Cancel', value: 'cancel' });
  
  const { friend } = await inquirer.prompt<FriendSelectPrompt>([{
    type: 'list',
    name: 'friend',
 ...
 */
async function interactiveExportKey(): Promise<InteractiveResult> {
 ...
 * Interactive send
 */
async function interactiveSend(): Promise<InteractiveSendResult> {
  // Get message
  const { message } = await inquirer.prompt<MessagePrompt>([{
    type: 'editor',
    name: 'message',
  ...
  // Get recipient
  const db = await listKeys();
  const choices: InteractiveChoice[] = [{ name: 'Yourself (self)', value: null as any }];
  ...
  const { recipient } = await inquirer.prompt<RecipientPrompt>([{
    type: 'list',
    name: 'recipient',
  ...
  // Get paste options
  const { isTemp } = await inquirer.prompt<TempPrompt>([{
    type: 'confirm',
    name: 'isTemp',
