...
// Load key database
export async function loadKeyDatabase() {
    await ensureDirectories();
    return initKeyDatabase();
}
// Save key database
export async function saveKeyDatabase(db) {
...
export async function addFriendKey(name, keyContent) {
...
// Get a specific key
export async function getKey(type, name) {
    const db = await loadKeyDatabase();
    if (type === 'self') {
        return db.keys.self;
    }
...
// Remove a key
export async function removeKey(type, name) {
    const db = await loadKeyDatabase();
...
export async function updateLastUsed(name) {
    const db = await loadKeyDatabase();
    if (db.keys.friends[name]) {
 ...
 */
export async function addPgpKey(name, keyInfo) {
 ...
 * @returns Path to the stored key
 */
export async function addKeybaseKey(name, keyInfo) {
